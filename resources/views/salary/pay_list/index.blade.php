    ...
    <section>
                        ...
                        <div class="col-md-12">
                            <form method="post" id="filter_form" class="form-horizontal" >
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label class="text-bold">{{trans('Payroll Cycle Of Sub Location')}}</label>
                                            <input type="text" readonly id="dynamic_payroll_cycle" class="form-control" value="Auto Fetch">
                                        </div>
                                    </div>
                                ...
                                <div class="text-center">
                                    <div class="form-group">
                                        <div class="form-actions">
                                            ...
                                            <button id="bulk_payment" onclick="makePayment()" type="button" class="filtering btn btn-primary"> <i class="fa fa-check-square-o"></i> {{__('BULK PAYMENT')}} </button>
                                        </div>
                                    </div>
                                </div>
                            </form>
        ...
        <div>
            <table id="pay_list-table" class="table table-responsive">
                ...
                <thead>
                    <tr>
                        <th class="not-exported">Select</th>
                    ...
                    <tr>
                        <th class="not-exported" data-orderable="false"><input type="checkbox" id="select_all_check"></th>
                        <th>{{trans('Sr. No.')}}</th>
                ...
                <tbody>
                    ...
                    @if(isset($employees) && count($employees) > 0)
                        @foreach($employees as $employee)
                            <tr>
                                <td><input type="checkbox" class="payment_employee" value="{{ $employee->id }}" ></td>
                                <td>{{ $index++ }}</td>
    ...
    <div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form action="{{ route('payroll.bulk_pay') }}" method="POST">
              @csrf
              <div class="modal-body">
                  <input type="hidden" name="employee_ids" id="payment_ids">
                  <input type="hidden" name="period" value="{{ isset($from) && isset($to) ? $from."/".$to:'' }}">
                  <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("#select_all_check").click(function() {
            ...
            $("input[class='payment_employee']").prop("checked", isChecked);
        });
    });
    
    function makePayment(){
        const selectedValues = $("input[class='payment_employee']:checked").map(function() {
            return $(this).val();
        }).get();

        // Check if at least one checkbox is selected
        if (selectedValues.length === 0) {
            alert("Please select at least one employee to make a payment.");
            return;
        }

        // Join the selected values into a comma-separated string
        const paymentIds = selectedValues.join(",");

        // Set the value of the payment_ids input field
        $("#payment_ids").val(paymentIds);
