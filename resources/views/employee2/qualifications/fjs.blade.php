    ...
    var table_table = $('#qualification-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('qualificationsf.show',$employee->id) }}",
        },
    ...
    $('#qualification_sample_form').on('submit', function (event) {
        ...
        if ($('#qualification_action').val() == '{{trans('file.Add')}}') {
            ...
            $.ajax({
                url: "{{ route('qualificationsf.store',$employee->id) }}",
                method: "POST",
        ...
        if ($('#qualification_action').val() == '{{trans('file.Edit')}}') {
            $.ajax({
                url: "{{ route('qualificationsf.update') }}",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
    ...
    $(document).on('click', '.qualification_edit', function () {
        ...
        var target = "{{ route('qualificationsf.index') }}/" + id + '/edit';
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $('#qualification_action_button').val('{{trans('file.Edit')}}');
    ...
    $('.qualification-ok').click(function () {
        let target = "{{ route('qualificationsf.index') }}/" + qualification_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
