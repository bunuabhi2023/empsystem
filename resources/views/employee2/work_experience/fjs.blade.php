    ...
    var table_table = $('#work_experience-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('work_experiencef.show',$employee->id) }}",
        },
    ...
    $('#work_experience_sample_form').on('submit', function (event) {
        ...
        if ($('#work_experience_action').val() == '{{trans('file.Add')}}') {
            ...
            $.ajax({
                url: "{{ route('work_experiencef.store',$employee->id) }}",
                method: "POST",
        ...
        if ($('#work_experience_action').val() == '{{trans('file.Edit')}}') {
            $.ajax({
                url: "{{ route('work_experiencef.update') }}",
                method: "POST",
                data: new FormData(this),
                contentType: false,
                cache: false,
    ...
    $(document).on('click', '.work_experience_edit', function () {
        ...
        var target = "{{ route('work_experiencef.index') }}/" + id + '/edit';
    ...
    $('.experience-ok').click(function () {
        let target = "{{ route('work_experiencef.index') }}/" + experience_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
