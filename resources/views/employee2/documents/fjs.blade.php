    ...
    var table_table = $('#document-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('documentsf.show',$employee->id) }}",
        },
    ...
    $('#document_sample_form').on('submit', function (event) {
        ...
        if ($('#document_action').val() == '{{trans('file.Add')}}') {
            ...
            $.ajax({
                url: "{{ route('documentsf.store',$employee->id) }}",
                method: "POST",
    ...
    $(document).on('click', '.document_edit', function () {
        ...
        var target = "{{ route('documentsf.index') }}/" + id + '/edit';
    ...
    $('.document-ok').click(function () {
        let target = "{{ route('documentsf.index') }}/" + document_delete_id + '/delete';
        $.ajax({
            url: target,
            beforeSend: function () {
