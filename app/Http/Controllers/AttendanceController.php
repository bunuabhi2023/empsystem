...
class AttendanceController extends Controller {
		...
		// {
			...
			if (request()->ajax())
			{
				...
				return datatables()->of($employee)
					...
					->addColumn('clock_in_pic', function ($employee)
					{
						if ($employee->employeeAttendance->isEmpty())
						{
							return '---';
						} else
						{
							...
							return '<a href="/public/'.$attendance_row->clock_in_pic.'" class="btn btn-link">View Image</a>';
						}
					})
					...
					->addColumn('clock_out_pic', function ($employee)
					{
						if ($employee->employeeAttendance->isEmpty())
						{
							return '---';
						} else
						{
							...
							return '<a href="/public/'.$attendance_row->clock_out_pic.'" class="btn btn-link">View Image</a>';
						}
					})
					...
					->rawColumns(['action', 'clock_in_pic', 'clock_out_pic'])
					->make(true);
			}
	...
	public function employeeAttendance(Request $request, $id)
	{
		...
		if (!$employee_attendance_last)
		{
            ...
            $image_parts = explode(";base64,", $request->input('in_out_pic'));
		...
		// {
			...
			if (request()->ajax())
			{
				...
				else
                {
                    ...
                    foreach ($employee as $key1 => $emp) {
                        ...
                        foreach ($date_range as $key2 => $dt_r) {
                            ...
                            if (array_key_exists($dt_r, $all_attendances_array))
                            {
                                $first = current($all_attendances_array[$dt_r])['clock_in_pic'];
                                $emp_attendance_date_range[$key1*count($date_range)+$key2]['clock_in_pic'] = $first;
                            }
                            else
                            {
                                $emp_attendance_date_range[$key1*count($date_range)+$key2]['clock_in_pic'] = '---';
                            }
                            //clock in pic
                            ...
                            if (array_key_exists($dt_r, $all_attendances_array))
                            {
                                $first = current($all_attendances_array[$dt_r])['clock_out_pic'];
                                $emp_attendance_date_range[$key1*count($date_range)+$key2]['clock_out_pic'] = $first;
                            }
                            else
                            {
                                $emp_attendance_date_range[$key1*count($date_range)+$key2]['clock_out_pic'] = '---';
                            }
                            //clock out pic
				...
				return datatables()->of($emp_attendance_date_range)
					...
					->addColumn('clock_in_pic', function ($row)
					{
					    if($row['clock_in_pic'] == '---'){
					        return '---';
					    }else{
						    return '<a href="/public/'.$row["clock_in_pic"].'" class="btn btn-link">View Image</a>';
					    }
					})
					...
					->addColumn('clock_out_pic', function ($row)
					{
					    if($row['clock_out_pic'] == '---'){
					        return '---';
					    }else{
						    return '<a href="/public/'.$row["clock_out_pic"].'" class="btn btn-link">View Image</a>';
					    }
					})
					...
					->rawColumns(['action','employee_name', 'clock_in_pic', 'clock_out_pic'])
					->make(true);
			}
...
// 		$last_date = date('Y-m-d', strtotime('last day of ' . $month_year));
		...
		$begin = new DateTime(date('Y-m-d', strtotime($first_date)));
		$end = new DateTime(date('Y-m-d', strtotime($last_date)));
		...
		// {
			if (request()->ajax())
			{
				if(!($logged_user->can('monthly-attendances'))) //Correction
				{
					...
					->select('id', 'company_id', 'first_name', 'last_name', 'office_shift_id', 'staff_id', 'sub_location_id', 'joining_date', 'exit_date', 'employee_type')
                    ->where('is_active',1)
                    ->where('exit_date',NULL)
				...
				else
				{
					...
					elseif (!empty($request->filter_group && $request->filter_company && $request->filter_loc && $request->filter_sub_loc))
					{
						$employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($first_date, $last_date)
						{
							$query->whereBetween('attendance_date', [$first_date, $last_date]);
							...
							->select('id', 'company_id', 'first_name', 'last_name', 'office_shift_id', 'staff_id', 'sub_location_id', 'joining_date', 'exit_date', 'employee_type')
							->where('sub_location_id', $request->filter_sub_loc)->where('is_active',1)
                            ->where('exit_date',NULL)->get();
					} 
					elseif (!empty($request->filter_group && $request->filter_company && $request->filter_loc))
					{
						$employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($first_date, $last_date)
						{
							$query->whereBetween('attendance_date', [$first_date, $last_date]);
							...
							->select('id', 'company_id', 'first_name', 'last_name', 'office_shift_id', 'staff_id', 'sub_location_id', 'joining_date', 'exit_date', 'employee_type')
							->where('location_id', $request->filter_loc)->where('is_active',1)
                            ->where('exit_date',NULL)->get();
					}
					elseif (!empty($request->filter_group && $request->filter_company))
					{
						$employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($first_date, $last_date)
						{
							$query->whereBetween('attendance_date', [$first_date, $last_date]);
					...
					elseif (!empty($request->filter_group))
					{
						$employee = Employee::with(['officeShift', 'employeeAttendance' => function ($query) use ($first_date, $last_date)
						{
							$query->whereBetween('attendance_date', [$first_date, $last_date]);
							...
							->select('id', 'client_grp_id', 'company_id', 'first_name', 'last_name', 'office_shift_id', 'staff_id', 'sub_location_id', 'joining_date', 'exit_date', 'employee_type')
							->where('client_grp_id', $request->filter_group)->where('is_active',1)
                            ->where('exit_date',NULL)->get();
					}
					else
					{
				...
				return datatables()->of($employee)
					...
					->addColumn('sub_loc_code', function ($row)
					{
						$code = $row->sub_location->location_code;
						return $code;
					...
					->addColumn('week_off', function ($row)
					{
						return $this->off_days;
					})
					->addColumn('tot_holiday', function ($row)
					{
						return $this->holidays;
					})
					->addColumn('tot_absent', function ($row)
					{
						return $this->absents;
					})
					->addColumn('tot_ded', function ($row)
					{
						return $this->add_ded;
					})
					->addColumn('total_adjust', function ($row){
					    return $this->total_adjustment;
					})
					->addColumn('net_total', function ($row)
					{
					    $totalOvertimeMinutes = 0;
						foreach ($row->employeeAttendance as $attendance_row)
						{
							sscanf($attendance_row->overtime, '%d:%d', $hour, $min);
							$totalOvertimeMinutes += $hour * 60 + $min;
						}
					    
					    $salary = EmployeeSalary::where('employee_id', $row->id)->first();
					    $salary['payable_days'] = $this->work_days + ($this->add_ded) + $this->holidays + $this->off_days + $this->total_adjustment;
					    $salary['total_overtime'] = $totalOvertimeMinutes;
					    $salary->save();
					    
					    
						return $this->work_days + ($this->add_ded) + $this->holidays + $this->off_days + $this->total_adjustment;
					})
			...
			return view('timesheet.monthlyAttendance.index', compact('client_groups'));
		// }
		// return response()->json(['success' => __('You are not authorized')]);
	}
	...
	public function checkAttendanceStatus($emp, $index)
	{
		...
		{
			$present = $emp->employeeAttendance->where('attendance_date', $this->date_attendance[$index])->values();
			...
			{
			    $attendanceDate = DateTime::createFromFormat('d-m-Y', $this->date_attendance[$index]);
                $joiningDate = DateTime::createFromFormat('d-m-Y', $emp->joining_date);
                $exitDate = DateTime::createFromFormat('d-m-Y', $emp->exit_date);
                
                if($joiningDate < $attendanceDate){
			        if($exitDate != ""){
			            if($exitDate > $attendanceDate){
        				    $this->total_adjustment += intval($present[0]->adjustment);
			    
            			    $this->present++;
            				// Get Shifts
                            $current_day_in = DateTime::createFromFormat('d-m-Y', $this->date_attendance[$index]);
                            $current_day_in = strtolower($current_day_in->format('l') . '_in');
                            $current_day_out = DateTime::createFromFormat('d-m-Y', $this->date_attendance[$index]);
                            $current_day_out = strtolower($current_day_out->format('l') . '_out');
                            
                            // Get Shifts Time
                            $shiftSTime = Carbon::createFromFormat('h:i A', $emp->officeShift->$current_day_in);
                            $shiftETime = Carbon::createFromFormat('h:i A', $emp->officeShift->$current_day_out);
                            // CLock in out time
                            $punchInTime = DateTime::createFromFormat('H:i', $present[0]->clock_in);
                            $punchOutTime = DateTime::createFromFormat('H:i', $present[0]->clock_out);
                            
                            // Calculate the difference in minutes
                            $timeSDifference = $punchInTime->diff($shiftSTime);
                            $timeEDifference = $punchOutTime->diff($shiftETime);
                            
                            // Calculate the total minutes, considering the sign
                            // list($hours, $minutes) = explode(':', $timeSDifference);
                            // $shiftStartMinutes = ($hours < 0 ? -1 : 1) * (abs($hours) * 60 + $minutes);
                            $shiftStartMinutes = $timeSDifference->h * 60 + $timeSDifference->i;
                            $shiftStartMinutes *= $timeSDifference->invert === 1 ? -1 : 1;
                            
                            $shiftEndMinutes = $timeEDifference->h * 60 + $timeEDifference->i;
                            $shiftEndMinutes *= $timeEDifference->invert === 1 ? -1 : 1;
                            
                            $data = (array) json_decode($emp->payroll->adeduction_settings);
                            
                            if ($shiftStartMinutes < 0) {
                                $payable = 0;
                                
                                foreach ($data['punch_in'] as $limit) {
                                    $min_value = intval($limit->min_value);
                                    $max_value = intval($limit->max_value);
                                    
                                    if ($shiftStartMinutes < 0) {
                                        if ($shiftStartMinutes <= $min_value && $shiftStartMinutes >= $max_value) {
                                            $payable += floatval($limit->payable);
                                            break; // Exit the loop as we found the matching range
                                        }
                                    } elseif ($shiftStartMinutes >= 0) {
                                        if ($shiftStartMinutes >= $min_value && $shiftStartMinutes <= $max_value) {
                                            $payable += floatval($limit->payable);
                                            break; // Exit the loop as we found the matching range
                                        }
                                ...
                                foreach ($data['punch_out'] as $limit) {
                                    $min_value = intval($limit->min_value);
                                    $max_value = intval($limit->max_value);
                                
                                    if ($shiftEndMinutes <= $min_value && $shiftEndMinutes >= $max_value) {
                                        $payable += floatval($limit->payable);
                                        break; // Exit the loop as we found the matching range
                                    }
		...
		if ($logged_user->can('edit-attendance'))
		{
			if (request()->ajax())
			{
				...
				return datatables()->of($employee_attendance)
					...
					->addColumn('manual_adjust', function ($row)
					{
	...
	public function attendanceHandler($request)
	{
        ...
        if (!$employee_attendance_last)
        {
            ...
            if ($clock_out > $shift_in || env('ENABLE_EARLY_CLOCKIN')!=NULL) {
                ...
                $data['adjustment'] = $request->manual_adjust;
            }
        }
        // if there is a record of employee attendance
        else {
