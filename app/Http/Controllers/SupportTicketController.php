...
class SupportTicketController extends Controller {
		...
		if ($logged_user->can('view-ticket'))
		{
			if (request()->ajax())
			{
				return datatables()->of(SupportTicket::with('company:id,company_name', 'employee:id,first_name,last_name', 'department:id,department_name')->get())
					...
					{
						$company_name = $row->sub_location->name ?? ' ';
						return $company_name;
					})
			...
			return view('SupportTicket.index', compact('client_groups'));
		}
	...
	public function store(Request $request)
	{
		...
		{
			$validator = Validator::make($request->only('subject', 'client_grp_id', 'location_id', 'sub_location_id', 'company_id', 'department_id', 'employee_id', 'ticket_priority', 'description', 'ticket_note'
				, 'ticket_attachments'),
				[
					'subject' => 'required',
