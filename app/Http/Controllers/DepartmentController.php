	...
	public function index()
	{
		...
		if (request()->ajax())
		{
			return datatables()->of(department::with('company:id,company_name', 'DepartmentHead:id,first_name,last_name')->get())
				...
				{
					return $row->company->company_name ?? 'All';
				})
	...
	public function store(Request $request)
	{
		...
		if ($logged_user->can('store-department'))
		{
			$validator = Validator::make($request->only('department_name', 'company_id', 'department_head'),
				[
					'department_name' => 'required|unique:departments,department_name,NULL,id',
					'common_dep' => 'nullable|boolean',
                    'company_id' => $request->common_dep ? 'nullable' : 'required'
				]
			);
			...
			if($request->common_dep){
			    $data['is_common'] = $request->common_dep;
			}
